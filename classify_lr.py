...
posdata = positiveData.map(lambda text : LabeledPoint(1, htf.transform(text.replace(',','').replace('.','').replace('-','').replace('?','').replace('!',' ').lower().split(" "))))
print "No. of Positive Sentences: " + str(posdata.count())
posdata.persist()

negativeData = sc.textFile("Negative.txt")
negdata = negativeData.map(lambda text : LabeledPoint(0, htf.transform(text.replace(',','').replace('.','').replace('-','').replace('?','').replace('!',' ').lower().split(" "))))
...
ptrain = posdata
ptest = pt.map(lambda text : LabeledPoint(1, htf.transform(text.replace(',','').replace('.','').replace('-','').replace('?','').replace('!','').lower().split(" "))))
ntrain = negdata
ntest = nt.map(lambda text : LabeledPoint(0, htf.transform(text.replace(',','').replace('.','').replace('-','').replace('?','').replace('!','').lower().split(" "))))
